---
hide:
  - toc
---

# :material-format-list-bulleted: Repository Registry

Browse the catalogue of **216** Power Platform & Copilot Studio
open-source repositories tracked by the Hub.

!!! info "Last synced: 2026-03-29 17:17 UTC"
    Data is refreshed by [`sync_repos.py`](https://github.com/rpothin/PowerPlatform-OpenSource-Hub/blob/main/scripts/sync_repos.py).

---

## Summary

| Metric | Value |
|--------|-------|
| **Total Repositories** | 216 |
| **Total Stars** | :star: 54,143 |
| **Open to Contributions** | 28 |

### Top Languages

| Language | Repositories |
|----------|-------------|
| TypeScript | 47 |
| Unknown | 41 |
| C# | 39 |
| PowerShell | 19 |
| JavaScript | 17 |
| Python | 14 |
| AL | 6 |
| HTML | 5 |
| R | 4 |
| CSS | 3 |

---

## Featured Repositories

<div class="grid cards" markdown>

-   :star: **skyvern** · 20,742 stars · `Python`

    ---

    Automate browser based workflows with AI

    [:octicons-arrow-right-24: View details](skyvern-ai_skyvern.md)

-   :star: **Power-Fx** · 3,339 stars · `C#`

    ---

    Power Fx low-code programming language

    [:octicons-arrow-right-24: View details](microsoft_power-fx.md)

-   :star: **portaljs** · 2,268 stars · `TypeScript`

    ---

    🌀 Rapidly build feature-rich data portals using a modern frontend framework. Native CKAN support. Op…

    [:octicons-arrow-right-24: View details](datopian_portaljs.md)

-   :star: **Microsoft365DSC** · 2,209 stars · `PowerShell`

    ---

    Manages, configures, extracts and monitors Microsoft 365 tenant configurations

    [:octicons-arrow-right-24: View details](microsoft_microsoft365dsc.md)

-   :star: **PowerApps-Samples** · 1,878 stars · `C#`

    ---

    Sample code for Power Apps, including Dataverse, model-driven apps, canvas apps, Power Apps componen…

    [:octicons-arrow-right-24: View details](microsoft_powerapps-samples.md)

-   :star: **Microsoft-Integration-and-Azure-Stencils-Pack-for-Visio** · 1,519 stars · `PowerShell`

    ---

    Microsoft Integration, Azure, Power Platform, Office 365 and much more Stencils Pack it’s a Visio pa…

    [:octicons-arrow-right-24: View details](sandroasp_microsoft-integration-and-azure-stencils-pack-for-visio.md)

-   :star: **PowerPlatformConnectors** · 1,216 stars · `C#`

    ---

    This is a repository for Microsoft Power Automate, Power Apps, and Azure Logic Apps connectors

    [:octicons-arrow-right-24: View details](microsoft_powerplatformconnectors.md)

-   :star: **power-pwn** · 1,133 stars · `Python`

    ---

    An offensive/defense security toolset for discovery, recon and ethical assessment of AI Agents

    [:octicons-arrow-right-24: View details](mbrg_power-pwn.md)

-   :star: **AL** · 843 stars · `PowerShell`

    ---

    Home of the Dynamics 365 Business Central AL Language extension for Visual Studio Code. Used to trac…

    [:octicons-arrow-right-24: View details](microsoft_al.md)

-   :star: **inoERP** · 829 stars · `JavaScript`

    ---

    inoERP is an OneApp [ Go back-end & Flutter front-end ] based enterprise management system

    [:octicons-arrow-right-24: View details](inoerp_inoerp.md)

-   :star: **BCTech** · 706 stars · `AL`

    ---

    Business Central technology samples

    [:octicons-arrow-right-24: View details](microsoft_bctech.md)

-   :star: **XrmToolBox** · 623 stars · `C#`

    ---

    An application that gather multiple tools to ease the customization and configuration of Microsoft D…

    [:octicons-arrow-right-24: View details](mscrmtools_xrmtoolbox.md)

-   :star: **viber3d** · 612 stars · `TypeScript`

    ---

    A modern starter kit for 3D browser games powered by r3f and threejs - 

    [:octicons-arrow-right-24: View details](instructa_viber3d.md)

-   :star: **PowerDocu** · 595 stars · `C#`

    ---

    Generate technical documentation from your existing Power Automate Flows and Power Apps canvas apps

    [:octicons-arrow-right-24: View details](modery_powerdocu.md)

-   :star: **PowerApps** · 537 stars · `Unknown`

    ---

    Collection of Power Apps template apps, components and examples

    [:octicons-arrow-right-24: View details](aprildunnam_powerapps.md)

-   :star: **powerapps-samples** · 521 stars · `Unknown`

    ---

    Contains curated community Microsoft Power App samples

    [:octicons-arrow-right-24: View details](pnp_powerapps-samples.md)

-   :star: **Dynamics-365-Workflow-Tools** · 500 stars · `C#`

    ---

    Dynamics 365 Workflow Tools is a Community solution that expands Microsoft Dynamics 365 (CRM) Workfl…

    [:octicons-arrow-right-24: View details](demianrasko_dynamics-365-workflow-tools.md)

-   :star: **alan-sdk-pcf** · 432 stars · `Unknown`

    ---

    The Self-Coding System for Your App — Alan AI SDK for Power Apps

    [:octicons-arrow-right-24: View details](alan-ai_alan-sdk-pcf.md)

-   :star: **PowerApps-Tooling** · 404 stars · `C#`

    ---

    Tooling support for PowerApps language and .msapp files 

    [:octicons-arrow-right-24: View details](microsoft_powerapps-tooling.md)

-   :star: **powercat-creator-kit** · 397 stars · `CSS`

    ---

    This toolkit helps create well-designed Power App experiences on the web & mobile. It contains a com…

    [:octicons-arrow-right-24: View details](microsoft_powercat-creator-kit.md)

-   :star: **powerplatform-samples** · 397 stars · `Unknown`

    ---

    The website for the PnP Power Platform samples gallery, including Power Apps, Power Automate, Power …

    [:octicons-arrow-right-24: View details](pnp_powerplatform-samples.md)

-   :star: **awesome-low-code** · 358 stars · `Unknown`

    ---

    Awesome Low-Code Application Platforms | 全球低代码平台开发资源大全

    [:octicons-arrow-right-24: View details](steedos_awesome-low-code.md)

-   :star: **powerautomate-samples** · 338 stars · `Unknown`

    ---

    Contains curated community samples for Microsoft Power Automate

    [:octicons-arrow-right-24: View details](pnp_powerautomate-samples.md)

-   :star: **PowerPlatform-DataverseServiceClient** · 307 stars · `C#`

    ---

    Code Replica for Microsoft.PowerPlatform.Dataverse.Client and supporting nuget packages. 

    [:octicons-arrow-right-24: View details](microsoft_powerplatform-dataverseserviceclient.md)

-   :star: **DynamicsWebApi** · 305 stars · `TypeScript`

    ---

    DynamicsWebApi is a Microsoft Dataverse Web API helper library for JavaScript & TypeScript

    [:octicons-arrow-right-24: View details](aleksandrrogov_dynamicswebapi.md)

-   :star: **teams-powerapps-app-templates** · 293 stars · `Unknown`

    ---

    Sample Teams app built by Power Apps

    [:octicons-arrow-right-24: View details](microsoft_teams-powerapps-app-templates.md)

-   :star: **office365dev** · 286 stars · `JavaScript`

    ---

    《Office 365 开发入门指南》，本书已经于2018年9月份由北京大学出版社上市发行，可以通过 http://product.dangdang.com/25347066.html 进行购买。配套…

    [:octicons-arrow-right-24: View details](chenxizhang_office365dev.md)

-   :star: **powerplatform-actions** · 273 stars · `TypeScript`

    ---

    Power Platform GitHub Actions automate common build and deployment tasks related to Power Platform. …

    [:octicons-arrow-right-24: View details](microsoft_powerplatform-actions.md)

-   :star: **powerplatform-build-tools** · 261 stars · `TypeScript`

    ---

    Power Platform Build Tools automate common build and deployment tasks related to Power Platform. Thi…

    [:octicons-arrow-right-24: View details](microsoft_powerplatform-build-tools.md)

-   :star: **Levelup-for-Dynamics-CRM** · 257 stars · `TypeScript`

    ---

    Chrome, Edge Chromium and Firefox Extension for Dynamics CRM/365/Power Apps Power users

    [:octicons-arrow-right-24: View details](rajyraman_levelup-for-dynamics-crm.md)

-   :star: **powerplatform-vscode** · 256 stars · `TypeScript`

    ---

    The Power Platform VSCode extension makes it easy to manage Power Platform environments and allows t…

    [:octicons-arrow-right-24: View details](microsoft_powerplatform-vscode.md)

-   :star: **microsoft-teams-apps-requestateam** · 239 stars · `PowerShell`

    ---

    Power Platform based solution that allows users to request teams and automates team creation. NO LON…

    [:octicons-arrow-right-24: View details](officedev_microsoft-teams-apps-requestateam.md)

-   :star: **powerapps-designtoolkit** · 218 stars · `Unknown`

    ---

    This toolkit is a curated repo of Power Apps UI Guidance and Components that you can plug and play i…

    [:octicons-arrow-right-24: View details](pnp_powerapps-designtoolkit.md)

-   :star: **Federal-Business-Applications** · 209 stars · `PowerShell`

    ---

    US Government Business Applications Demo Repository

    [:octicons-arrow-right-24: View details](microsoft_federal-business-applications.md)

-   :star: **powerplatform-snippets** · 207 stars · `HLSL`

    ---

    Repository with snippets that can easily be used in some of the Power Platform products

    [:octicons-arrow-right-24: View details](pnp_powerplatform-snippets.md)

-   :star: **xrm-ci-framework** · 194 stars · `C#`

    ---

    xRM CI Framework provides you with the tools automate the build and deployment of your CRM Solution.…

    [:octicons-arrow-right-24: View details](waelhamze_xrm-ci-framework.md)

-   :star: **raaghu-react** · 187 stars · `TypeScript`

    ---

    Raaghu Design System – React is an open-source design system for modern Web and Power Apps applicati…

    [:octicons-arrow-right-24: View details](wai-technologies_raaghu-react.md)

-   :star: **MicrosoftCloudLogos** · 184 stars · `Python`

    ---

    A collection of Microsoft cloud product logos

    [:octicons-arrow-right-24: View details](loryanstrant_microsoftcloudlogos.md)

-   :star: **PowerPlatformAdvocates** · 183 stars · `Unknown`

    ---

    This repository contains presentations and workshops created by the Microsoft Power Platform Advocat…

    [:octicons-arrow-right-24: View details](microsoft_powerplatformadvocates.md)

-   :star: **Low-Code** · 182 stars · `JavaScript`

    ---

    Website to showcase Low-Code content and community initiatives to help developers explore low code a…

    [:octicons-arrow-right-24: View details](microsoft_low-code.md)

-   :star: **30daysof** · 181 stars · `JavaScript`

    ---

    30 Day of Learning Resources, Samples and Curricula

    [:octicons-arrow-right-24: View details](microsoft_30daysof.md)

-   :star: **powerplatform-actions-lab** · 173 stars · `Unknown`

    ---

    This repo contains sample workflow definitions for GitHub Actions for Microsoft Power Platform.

    [:octicons-arrow-right-24: View details](microsoft_powerplatform-actions-lab.md)

-   :star: **hub** · 169 stars · `MDX`

    ---

    Everything you need to navigate the certification world.

    [:octicons-arrow-right-24: View details](mscerts_hub.md)

-   :star: **FlowPowerAppsMigrator** · 156 stars · `PowerShell`

    ---

    Converts SharePoint Data sources for the MS Flow and PowerApps to a new tenant of your choice.

    [:octicons-arrow-right-24: View details](zerg00s_flowpowerappsmigrator.md)

-   :star: **Project-Accelerator** · 145 stars · `Unknown`

    ---

    An Accelerator solution for Project for the web

    [:octicons-arrow-right-24: View details](officedev_project-accelerator.md)

-   :star: **XrmDefinitelyTyped** · 142 stars · `TypeScript`

    ---

    Tool to generate TypeScript declaration files for Dynamics 365/CDS client-side coding.

    [:octicons-arrow-right-24: View details](delegateas_xrmdefinitelytyped.md)

-   :star: **powerfx-samples** · 135 stars · `Unknown`

    ---

    This repository contains samples for Power Fx low-code programming language https://github.com/micro…

    [:octicons-arrow-right-24: View details](pnp_powerfx-samples.md)

-   :star: **PowerPlatformAF** · 135 stars · `Unknown`

    ---

    Power Platform Adoption Framework

    [:octicons-arrow-right-24: View details](powerplatformaf_powerplatformaf.md)

-   :star: **DRB** · 121 stars · `JavaScript`

    ---

    Dataverse REST Builder

    [:octicons-arrow-right-24: View details](guidopreite_drb.md)

-   :star: **dyn365-ce-vsts-tasks** · 121 stars · `PowerShell`

    ---

    VSTS Extension for Dynamics 365 Customer Engagement

    [:octicons-arrow-right-24: View details](waelhamze_dyn365-ce-vsts-tasks.md)

</div>

---

## All Repositories

??? note "View all remaining repositories"

    | Repository | Language | Stars | Description |
    |------------|----------|-------|-------------|
    | [evoke](cheeseburgertony_evoke.md) | TypeScript | :star: 121 | AI-powered web app generator — turn natural language ideas into full code with l… |
    | [PowerAutomate](damobird365_powerautomate.md) | — | :star: 117 | Power Automate demos & proof of concepts |
    | [awesome-power-platform](power-maverick_awesome-power-platform.md) | — | :star: 111 | A collection of awesome things regarding the Power Platform ecosystem. |
    | [OrgDbOrgSettings](seanmcne_orgdborgsettings.md) | HTML | :star: 110 | This is a utility allowing admins of Dynamics CRM Online & onPrem to edit organi… |
    | [php-crm-toolkit](alexacrm_php-crm-toolkit.md) | PHP | :star: 107 | Dynamics CRM Toolkit for PHP |
    | [awesome-power-automate-learning-resources](iamericfletcher_awesome-power-automate-learning-resources.md) | — | :star: 107 | A curated collection of learning resources to help deepen your understanding of … |
    | [power-automate-desktop-framework](agniusbartninkas_power-automate-desktop-framework.md) | TSQL | :star: 107 | Framework for having standardized structure of Microsoft Power Automate Desktop … |
    | [provision-assist-m365](pnp_provision-assist-m365.md) | PowerShell | :star: 107 | Provision Assist is a request and provisioning solution for Teams, Groups, SPO S… |
    | [Xrm-WebApi-Client](xrm-oss_xrm-webapi-client.md) | JavaScript | :star: 104 | A promise-based JavaScript library for the Microsoft Dynamics CRM WebApi - TypeS… |
    | [ProvisionGenie](provisiongenie_provisiongenie.md) | Bicep | :star: 103 | ProvisionGenie 🧞 is an open-source project by Luise Freese & Carmen Ysewijn. We … |
    | [powerplatform](microsoft_powerplatform.md) | — | :star: 100 | The official repository for Power Platform tools and resources. |
    | [aotbrowser](arganollc_aotbrowser.md) | C# | :star: 97 | Dynamics 365 for Finance and Operations AOT Browser |
    | [Power-CAT-Tools](microsoft_power-cat-tools.md) | C# | :star: 96 | The Power CAT Tools is your ultimate companion for enhancing the development exp… |
    | [dyn365-ce-devops](devkeydet_dyn365-ce-devops.md) | — | :star: 94 | DevOps for Dynamics 365 Customer Engagement (CE) is becoming a popular topic.  T… |
    | [D365FONinjaDevTools](hichemdax_d365foninjadevtools.md) | C# | :star: 87 | To make of you a Ninja Developer in Dynamics 365 For Finance and Operations |
    | [datahugger](j535d165_datahugger.md) | Python | :star: 85 | One downloader for many scientific data and code repositories! DOI :open_hands: … |
    | [PowerApps-samples](eickhel_powerapps-samples.md) | TypeScript | :star: 84 | PowerApps samples |
    | [Planner-PowerBI-with-PowerAutomate](ben-howard_planner-powerbi-with-powerautomate.md) | — | :star: 83 | Contains a PowerAutomate file that takes data from Planner, writes it to Excel, … |
    | [xrm-mock](davidjbclark_xrm-mock.md) | TypeScript | :star: 83 | :books: A fake implementation of the Xrm object model. Written in TypeScript aga… |
    | [XrmMockup](delegateas_xrmmockup.md) | C# | :star: 81 | Engine which simulates your exact Dynamics 365/CRM instance locally including al… |
    | [DataverseDevTools-VSCode](power-maverick_dataversedevtools-vscode.md) | TypeScript | :star: 81 | The all-in-one tool to develop code for Dataverse/Dynamics 365. Helps you connec… |
    | [dynamics-webapi-toolkit](alexacrm_dynamics-webapi-toolkit.md) | PHP | :star: 81 | Dynamics 365 Web API Toolkit for PHP |
    | [pyDataverse](gdcc_pydataverse.md) | Python | :star: 80 | Python module for Dataverse Software (dataverse.org). |
    | [RibbonWorkbench](scottdurow_ribbonworkbench.md) | JavaScript | :star: 76 | Ribbon Workbench 2016 for Dynamics 365 |
    | [crm-power-pane](onurmenal_crm-power-pane.md) | JavaScript | :star: 76 | The CRM Power Pane is a helper tool designed to integrate with Microsoft Dynamic… |
    | [PowerApps-Branding-Template](iammancat_powerapps-branding-template.md) | PowerShell | :star: 75 | A template to empower developers and reduce time wasted on control changes |
    | [Dynamics-Crm-DevKit](phuocle_dynamics-crm-devkit.md) | C# | :star: 75 | A collection of projects/items template for develop Dynamics 365 CRM/CDS/Dataver… |
    | [Templates-for-Power-Platform](microsoft_templates-for-power-platform.md) | C# | :star: 75 | Solutions designed to expedite the implementation of Power Platform within large… |
    | [dataverse-ify](scottdurow_dataverse-ify.md) | TypeScript | :star: 73 | Easily call the Dataverse WebApi from TypeScript using SDK style types, with a N… |
    | [developerkorea](microsoft_developerkorea.md) | C# | :star: 68 | 마이크로소프트 기술 블로그 한국어판 | Microsoft tech blogs in Korean |
    | [PCF-Controls](power-maverick_pcf-controls.md) | TypeScript | :star: 67 | Repos of Powerapps Component Framework (PCF) Controls |
    | [WarehouseControlSystem](oleglobakov_warehousecontrolsystem.md) | C# | :star: 66 | Warehouse Control System for Microsoft Dynamics NAV |
    | [dataverse-client-r](iqss_dataverse-client-r.md) | R | :star: 65 | R Client for Dataverse Repositories |
    | [odata2poco](moh-hassan_odata2poco.md) | C# | :star: 65 | generate POCO classes from OData service |
    | [Power-BI-D365-Sales](mscottsewell_power-bi-d365-sales.md) | TSQL | :star: 65 | Power BI ❤️ Dynamics 365 - Sales Report and ALM Solution Examples for Power BI a… |
    | [power-platform-deprecation-tracker](tcorcor1_power-platform-deprecation-tracker.md) | — | :star: 62 | Repo used for tracking changes on various Power Platform deprecation pages |
    | [pcf-universal-gantt-chart](matematuk_pcf-universal-gantt-chart.md) | TypeScript | :star: 61 | Interactive PCF Gantt Chart Control for Dynamics Grid |
    | [PowerFxSnippets](powerappsdarren_powerfxsnippets.md) | Python | :star: 57 | My bag of tricks for the Power Platform, mostly canvas Power Apps (PowerFx & YAM… |
    | [TrulyResponsive](feincraft_trulyresponsive.md) | — | :star: 55 | TrulyResponsive is the framework to build PowerApps that show the right UX on an… |
    | [geoflow](r-geoflow_geoflow.md) | R | :star: 54 | Orchestrate Geospatial (Meta)Data Management Workflows and Manage FAIR Services |
    | [PowerApps-Azure-Lab](microsoft_powerapps-azure-lab.md) | C# | :star: 53 | Repository to host the PowerApps Azure Lab |
    | [dataverse-erd-visualizer](allandecastro_dataverse-erd-visualizer.md) | TypeScript | :star: 51 |  |
    | [FlowToVisio](linked365_flowtovisio.md) | C# | :star: 50 | XrmToolBox tool to allow you to document your cloud flows within Microsoft Power… |
    | [terraform-provider-power-platform](microsoft_terraform-provider-power-platform.md) | Go | :star: 49 | Power Platform Terraform Provider |
    | [SSD365VSAddIn](shashisadasivan_ssd365vsaddin.md) | C# | :star: 48 | Visual studio addin for D365 Finance and operations |
    | [development-hub](ewingjm_development-hub.md) | C# | :star: 45 | A continuous integration solution for Power Apps. |
    | [powerapps-docstring](sebastian-muthwill_powerapps-docstring.md) | Python | :star: 45 | PowerApps-docstring is a console based, pipeline ready application that automati… |
    | [PCF-CustomControlBuilder](power-maverick_pcf-customcontrolbuilder.md) | C# | :star: 44 | XrmToolBox Plugin for building and deploying custom control using PCF |
    | [AliFullStack](sfarpak_alifullstack.md) | TypeScript | :star: 44 | Free Local-first Full-Stack AI App Builder & Automation — Build, Test & Deploy w… |
    | [dynamics365crm-python](gearplug_dynamics365crm-python.md) | Python | :star: 44 | Dynamics365CRM API wrapper written in Python. |
    | [D365FFO-PowerShell-scripts](valerymoskalenko_d365ffo-powershell-scripts.md) | PowerShell | :star: 42 | Set of PowerShell scripts to maintain D365FFO (Dynamics 365 for Finance and Oper… |
    | [teta](ondateta_teta.md) | Dart | :star: 41 | The open source, AI-powered app builder |
    | [Power-Platform](wyattdave_power-platform.md) | HTML | :star: 40 | Collection of examples and demos from my Power Platform Blog |
    | [power-platform-skills](microsoft_power-platform-skills.md) | TypeScript | :star: 38 | A plugin marketplace for Claude Code/GitHub Copilot that provides Power Platform… |
    | [bcsamples-takeorder](microsoft_bcsamples-takeorder.md) | AL | :star: 37 | The Take Order sample demonstrates how you can build a customized user experienc… |
    | [PowerDataOps](aymericm78_powerdataops.md) | PowerShell | :star: 37 | PowerShell module for Microsoft Dataverse and Power Platform automation (Data Ma… |
    | [pac2](ntt-security-japan_pac2.md) | Python | :star: 36 | Power Automate C2 (PAC2) :  Stealth living-off-the-cloud C2 framework. |
    | [blog](srenders_blog.md) | AL | :star: 35 | Examples from my blog |
    | [SmartBrief](rkneela0912_smartbrief.md) | — | :star: 35 | A low-code open source solution for document summarization using Microsoft Power… |
    | [fake-xrm-easy](dynamicsvalue_fake-xrm-easy.md) | C# | :star: 35 | FakeXrmEasy: The Test Automation Framework for the Power Platform / Dataverse |
    | [PowerProgressive](feincraft_powerprogressive.md) | — | :star: 34 | Run #PowerApps as native desktop apps with this PWA conversion kit |
    | [vsc-xliff-sync](rvanbekkum_vsc-xliff-sync.md) | TypeScript | :star: 34 | Visual Studio Code extension that allows for automatic merging of (simple) XLIFF… |
    | [BC-OAuth-2.0-Authorization](msnraju_bc-oauth-2.0-authorization.md) | Perl | :star: 34 | Generic OAuth2 Library for Business Central |
    | [JonasPluginBase](rappen_jonaspluginbase.md) | C# | :star: 33 | Microsoft Dynamics CRM / 365 plugin base class |
    | [ChartComponents](ogcanviz_chartcomponents.md) | — | :star: 32 | SVG chart components for Microsoft Power Apps. |
    | [open-ai-teams-chat-bot](zerg00s_open-ai-teams-chat-bot.md) | — | :star: 32 | This Github repository contains two Power Automate flows that use OpenAI to answ… |
    | [powerapps-project-template](capgemini_powerapps-project-template.md) | C# | :star: 32 | A Yeoman generator for scaffolding Power Apps projects based on our tooling and … |
    | [icews](andybega_icews.md) | R | :star: 30 | Get the ICEWS event data |
    | [PowerAppsScripts](denise-msft_powerappsscripts.md) | PowerShell | :star: 30 | PowerShell scripts for PowerApps and Flow automation |
    | [bcsamples-warehousehelper](microsoft_bcsamples-warehousehelper.md) | AL | :star: 30 | The Warehouse Helper sample consists of an AL extension, a Power App, and a Powe… |
    | [SLG-Business-Applications](microsoft_slg-business-applications.md) | — | :star: 29 | A collection of Power Platform and Dynamics 365 resources, open-sourced to the S… |
    | [PCF-AttachmentGalleryControl](oolashyn_pcf-attachmentgallerycontrol.md) | TypeScript | :star: 29 | Attachment Gallery Control using Powerapps Component Framework |
    | [Power-Platform-Samples](valentinmaz_power-platform-samples.md) | — | :star: 29 | This repository contains Power Platform samples and templates that I share with … |
    | [PowerPlatform-OpenSource-Hub](rpothin_powerplatform-opensource-hub.md) | TypeScript | :star: 29 | Your amazing journey in Power Platform open-source ecosystem could start here 🧳 |
    | [Crm.Specflow](mvermaat_crm.specflow.md) | C# | :star: 29 | D365 Extension for SpecFlow allows you to automatically test your Dynamics 365 C… |
    | [XrmFramework](dimsi-bs_xrmframework.md) | C# | :star: 28 | Xrm Framework |
    | [xrm-webapi](derekfinlinson_xrm-webapi.md) | TypeScript | :star: 28 | Dynamics 365 Web Api TypeScript module |
    | [fscps](fscpscollaborative_fscps.md) | PowerShell | :star: 28 | D365 FSC DevOps setup&maintenance tools for GitHub |
    | [easyDataverse](gdcc_easydataverse.md) | Python | :star: 28 | 🪐 - Lightweight Dataverse interface in Python to upload, download and update dat… |
    | [xrm-datamigration](capgemini_xrm-datamigration.md) | C# | :star: 27 | Export and import data for Microsoft Dataverse. Supports JSON and CSV. |
    | [powerapps-packagedeployer-template](capgemini_powerapps-packagedeployer-template.md) | C# | :star: 27 | Enhanced deployment capabilities when deploying with the Power Apps Package Depl… |
    | [PyConnectDataverse](yeswecandrew_pyconnectdataverse.md) | Python | :star: 27 | Connect to the Dynamics 365 / Dataverse Web API using interactive login |
    | [Dataverse-Data-Explorer](scholarsportal_dataverse-data-explorer.md) | TypeScript | :star: 26 | A tool for exploring and curating data by adding labels, groups, weights and oth… |
    | [FetchXmlTester](ashv_fetchxmltester.md) | JavaScript | :star: 26 | Test FetchXml Online without providing credentials using WebAPI GET Call |
    | [dataverse-gen](scottdurow_dataverse-gen.md) | TypeScript | :star: 25 | Early-bound types generator for CDS/Dataverse |
    | [Dynamics-365-Toolkit](gotdibbs_dynamics-365-toolkit.md) | JavaScript | :star: 25 | Toolkit for development in a Microsoft Dynamics 365/CRM 2011+ environment. Note … |
    | [xrmtools](rezanid_xrmtools.md) | C# | :star: 25 | Xrm Tools brings all the missing features for Power Platform to Visual Studio, o… |
    | [erpl-web](datazoode_erpl-web.md) | C++ | :star: 25 | ERPL is a DuckDB extension to connect to API based ecosystems via standard inter… |
    | [TACO](officedev_taco.md) | PowerShell | :star: 25 | Microsoft Teams AA/CQ Orchestrator (TACO) App |
    | [inline-query](msnraju_inline-query.md) | AL | :star: 25 | Inline Query | SQL like Queries in Business Central |
    | [PCF-Builder-VSCode](power-maverick_pcf-builder-vscode.md) | JavaScript | :star: 25 | Build your Power Apps Component Framework custom controls faster. No need to rem… |
    | [microsoft-teams-apps-request-a-guest](officedev_microsoft-teams-apps-request-a-guest.md) | PowerShell | :star: 24 | Power Apps solution that automates the guest approval and invite process |
    | [prompt-pulse](pnp_prompt-pulse.md) | — | :star: 24 | Prompt Pulse is a free Microsoft Teams Power App enabling users to schedule and … |
    | [D365-CE-DevOps-Tutorial](microsoft-d365-ce-pfe-devops_d365-ce-devops-tutorial.md) | JavaScript | :star: 23 | Dynamics 365 Customer Engagement DevOps tutorial material repository |
    | [powerplatform-vscode-lab](microsoft_powerplatform-vscode-lab.md) | TypeScript | :star: 23 | Hands-on lab for Power Platform Tools for Visual Studio Code |
    | [ps-xliff-sync](rvanbekkum_ps-xliff-sync.md) | PowerShell | :star: 22 | A PowerShell module to synchronize and validate XLIFF translation files. |
    | [integration-dynamics](alexacrm_integration-dynamics.md) | PHP | :star: 22 | The easiest way to connect Dynamics 365 with WordPress. |
    | [UNF](leeper_unf.md) | R | :star: 22 | Tools for Creating Universal Numeric Fingerprints for Data |
    | [harvesterjs](datopian_harvesterjs.md) | TypeScript | :star: 20 | Framework and scripts for harvesting datasets into PortalJS . Provides reusable … |
    | [d365fo-mcp-server](dynamics365ninja_d365fo-mcp-server.md) | TypeScript | :star: 20 | 🚀 MCP server for Dynamics 365 Finance & Operations with standard and custom inde… |
    | [PowerApps-Managed-Identity-Demo-Functions](rajyraman_powerapps-managed-identity-demo-functions.md) | Bicep | :star: 20 | Demo repository to show how to use Managed Identity on Azure Functions to connec… |
    | [powerplatformconnect](microsoft_powerplatformconnect.md) | HTML | :star: 20 | Materiais, tutoriais e dicas sobre Power Platform criados pela equipe de especia… |
    | [PowerPlatform-DataverseModelBuilder](microsoft_powerplatform-dataversemodelbuilder.md) | C# | :star: 20 | Code Replica for the Microsoft Power Platform Dataverse Model Builder and suppor… |
    | [powerplatform-cli-wrapper](microsoft_powerplatform-cli-wrapper.md) | TypeScript | :star: 20 | Power Platform CLI Wrapper |
    | [control-add-in-samples](msnraju_control-add-in-samples.md) | Perl | :star: 20 | Microsoft Dynamics 365 Business Central - Control Add-in Samples |
    | [eBusinessCard](experience-digital-ms_ebusinesscard.md) | — | :star: 19 | Contactless Business Card: This component adds a user profile page in your app. … |
    | [Rnwood.Dataverse.Data.PowerShell](rnwood_rnwood.dataverse.data.powershell.md) | C# | :star: 19 | A PowerShell module for interacting with Microsoft Dataverse. |
    | [release-plans](jukkan_release-plans.md) | HTML | :star: 19 | Release Plans Visualized - Power BI report and website for browsing release item… |
    | [Power-BI-Read-Only-REST-API](ferrybouwman_power-bi-read-only-rest-api.md) | PowerShell | :star: 19 | Solution based on either Powershell or Power Automate (Flow) to consume the rece… |
    | [PowerPlatform-VSCode-ExtensionPack](power-maverick_powerplatform-vscode-extensionpack.md) | — | :star: 19 | The ultimate collection of extensions for Power Platform! |
    | [dataverse-uploader](iqss_dataverse-uploader.md) | Python | :star: 19 | GitHub Action to publish repository content on Dataverse |
    | [PCF-Gallery-Control](dynamicsninja_pcf-gallery-control.md) | TypeScript | :star: 19 | PCF control that shows thumbnails of images from entity notes. Control is bound … |
    | [Dataverse-Dialog-Builder](phuocle_dataverse-dialog-builder.md) | JavaScript | :star: 19 | A managed solution helps you build Microsoft Dataverse OOB dialogs, such as the … |
    | [CanvasTimelinePCF](belleye_canvastimelinepcf.md) | TypeScript | :star: 18 | Power Apps Canvas PCF based on vis-timeline |
    | [paca-modern-web-app-sample](nunosubtil_paca-modern-web-app-sample.md) | TypeScript | :star: 18 | Modern Web App Sample for Power Apps Code Apps |
    | [microsoft-teams-apps-call-queue-scheduler](officedev_microsoft-teams-apps-call-queue-scheduler.md) | PowerShell | :star: 18 | Microsoft Teams Call Queue Manager app template |
    | [cces_cumulative](kuriwaki_cces_cumulative.md) | TeX | :star: 18 | Building a cumulative file (2006 - 2023) for the Cooperative Congressional Elect… |
    | [group-approval-viewer-D365-pcf](tcorcor1_group-approval-viewer-d365-pcf.md) | TypeScript | :star: 18 | Dynamics 365 PCF control for displaying the status of Power Automate group appro… |
    | [enhanced-flow-command-set](cupo365_enhanced-flow-command-set.md) | TypeScript | :star: 17 | Trigger a Power Automate or Azure Logic App flow from SharePoint while selecting… |
    | [Xrm-Quick-Edit](xrm-oss_xrm-quick-edit.md) | JavaScript | :star: 17 | A Dynamics CRM Add-In for speeding up tasks such as translating or toggling fiel… |
    | [orchestrator_rpa](daferferso_orchestrator_rpa.md) | CSS | :star: 16 | Orchestrator RPA is a Django-based platform for managing tasks with automation r… |
    | [power-bookmarklets](jukkan_power-bookmarklets.md) | JavaScript | :star: 16 | Power Platform development and administration scripts to be run as browser bookm… |
    | [SharePointWorkflowConverter](dev-mom_sharepointworkflowconverter.md) | C# | :star: 16 | A proof-of-concept C# console application for retrieving legacy 2010 & 2013 cust… |
    | [modal](modal-dsl_modal.md) | Xtend | :star: 16 | A Domain Specific Language for AL Extension Development |
    | [Power-Platform-App-Dev-in-a-Day-KR](microsoft_power-platform-app-dev-in-a-day-kr.md) | Bicep | :star: 16 | Power Platform Hands-on-Lab sessions repository in Korean |
    | [powerux](amplifyit_powerux.md) | — | :star: 15 | PowerUX is a no-code UX extender for model-driven apps, designed to empower low-… |
    | [sp-document-explorer](msnraju_sp-document-explorer.md) | Perl | :star: 15 | Handling SharePoint Documents in Business Central |
    | [primno](primno_primno.md) | TypeScript | :star: 15 | Front-end framework for Model-Driven Apps of Power Apps and Dynamics 365 |
    | [agents-pro](timothywarner-org_agents-pro.md) | Python | :star: 15 | Teaching/learning resources for O'Reilly Live 'How to Create AI Agents Like a Pr… |
    | [dataverse-auth](scottdurow_dataverse-auth.md) | TypeScript | :star: 15 | Performs on-behalf-of auth against a Microsoft Dataverse environment using pure … |
    | [dataverse-mcp](mwhesse_dataverse-mcp.md) | TypeScript | :star: 15 | A Model Context Protocol (MCP) server for Microsoft Dataverse that enables schem… |
    | [PCF-Components](benlbartle_pcf-components.md) | TypeScript | :star: 15 | PowerApp Component Framework, erm, Components |
    | [superez-gpt-builder](g023_superez-gpt-builder.md) | JavaScript | :star: 14 | SuperEZ-GPT Builder: jQuery/GPT AI Content Building Assistant |
    | [ej2-powerapps-components](syncfusion_ej2-powerapps-components.md) | TypeScript | :star: 14 | This repository features Syncfusion PowerApps components built using the PowerAp… |
    | [global-copilot-summit](microsoft_global-copilot-summit.md) | — | :star: 14 | The theme of the summit is Adopt, Extend, and Build Copilot, and is attended by … |
    | [prompt-master](pnp_prompt-master.md) | — | :star: 14 | Prompt Master and Prompt Master Pro are free Power Apps for gamifying Microsoft … |
    | [PCFControls](bidsl_pcfcontrols.md) | TypeScript | :star: 14 | PCF controls for Model Driven Apps |
    | [OneMoreActionQueueFramework](sminozhenko_onemoreactionqueueframework.md) | — | :star: 14 | Dynamics 365 For Finance and Operations action queue framework |
    | [PowerApps-Guidelines](automatevalue_powerapps-guidelines.md) | JavaScript | :star: 13 | Coding standards and Guidelines for Customizing and Extending PowerApps & Dynami… |
    | [PowerNote](scalable-dynamics_powernote.md) | C# | :star: 13 | Power Fx editor and file viewer |
    | [playwright-m365-helpers](estruyf_playwright-m365-helpers.md) | TypeScript | :star: 13 | A set of helpers for creating E2E tests for your Microsoft 365 projects using Pl… |
    | [Microsoft-Power-Platform-In-a-Day-Workshop](rramoscabral_microsoft-power-platform-in-a-day-workshop.md) | — | :star: 13 | Microsoft Power Platform In a Day Workshop |
    | [D365-CE-Pipelines](microsoft-d365-ce-pfe-devops_d365-ce-pipelines.md) | — | :star: 12 | Collection of YAML files for building Azure DevOps pipelines for Dynamics 365 fo… |
    | [BCDevApps](picazin_bcdevapps.md) | AL | :star: 12 | Tools to improve and make BC developers easy |
    | [dynamics-client](mrthearman_dynamics-client.md) | Python | :star: 12 | Client for making Web API request from a Microsoft Dynamics 365 Database |
    | [mssecuritynews](vladjoh_mssecuritynews.md) | — | :star: 12 | A set of Power Automate flows that fetch the latest Microsoft Security News from… |
    | [MSFTCloudSvcsFavs](timdarius_msftcloudsvcsfavs.md) | HTML | :star: 12 | This repo contains a collection of useful links to Microsoft Cloud Services that… |
    | [ChartJsCanvasPCF](belleye_chartjscanvaspcf.md) | TypeScript | :star: 12 | Chart JS in PowerApps |
    | [Power-Apps-Toast-Notification-Component](erfernandes_power-apps-toast-notification-component.md) | — | :star: 11 | Component created for toast style notifications to be used in Power Apps Canvas … |
    | [dataverse-powertools](pete-mc_dataverse-powertools.md) | TypeScript | :star: 11 | Dataverse Powertools contains tools and templates to help with the development o… |
    | [booking-powerapp](ans-group_booking-powerapp.md) | TSQL | :star: 11 | Booking Application built with Microsoft PowerApps |
    | [Simple-Text-Control-PCF](kokulan365_simple-text-control-pcf.md) | TypeScript | :star: 11 | A simple yet very flexible PCF control to display text with custom style using H… |
    | [MsCrmTools.MetadataDocumentGenerator](mscrmtools_mscrmtools.metadatadocumentgenerator.md) | C# | :star: 10 | An XrmToolBox plugin to generate Excel documentation with CRM metadata |
    | [cdh-lava-react](cdcgov_cdh-lava-react.md) | CSS | :star: 10 | CDC Data Hub Lifecycle, Analysis & Visualization Accelerator (LAVA) REACT Compon… |
    | [Minimal-React-PCF](paulbreuler_minimal-react-pcf.md) | TypeScript | :star: 10 | A minimal React based PowerApps Control Framework (PCF) template  |
    | [D365RichTextEditor](chrismayor_d365richtexteditor.md) | TypeScript | :star: 10 | Dynamics 365 Rich text editor for Unified Interface / Based on PowerApps compone… |
    | [mcp-dataverse](rajyraman_mcp-dataverse.md) | C# | :star: 10 | MCP Server for querying Dataverse using SQL |
    | [portal-helper-vscode](oolashyn_portal-helper-vscode.md) | TypeScript | :star: 10 | Extension for VS Code to help to work with Power Apps CLI for Portals |
    | [Stock-Templates-Remastered](powerappsdarren_stock-templates-remastered.md) | Shell | :star: 10 | Microsoft Power Apps has several stock templates. We'll dust them off and make s… |
    | [PowerAutomateMockUp](thygesteffensen_powerautomatemockup.md) | C# | :star: 10 | Skeleton to run Power Automate Flows from their JSON descriptions. |
    | [VisitorManagement](teamruegg_visitormanagement.md) | — | :star: 10 | The Visitor Management is a collection of four Power Apps to manage smart factor… |
    | [fscps.tools](fscpscollaborative_fscps.tools.md) | PowerShell | :star: 10 | A set of tools for maintaining D365FSC CI/CD pipelines. |
    | [xrm-solutionaudit](capgemini_xrm-solutionaudit.md) | C# | :star: 6 | This is an open-source project which aims to facilitate the audit of Dynamics 36… |
    | [Decommissioned-emcr-ess-mod](bcgov_decommissioned-emcr-ess-mod.md) | — | :star: 6 | Streamline processes so evacuated individuals can access services easily and eff… |
    | [dataverse-pm](iqss_dataverse-pm.md) | — | :star: 4 | Project management issue tracker for the Dataverse Project. Note: Related links … |
    | [dataverse-helper](dvla_dataverse-helper.md) | Ruby | :star: 3 | This gem helps you integrate with Microsoft Dynamics using Microsoft Dataverse W… |

---

_Auto-generated by [`sync_repos.py`](https://github.com/rpothin/PowerPlatform-OpenSource-Hub/blob/main/scripts/sync_repos.py) on 2026-03-29 17:17 UTC._
